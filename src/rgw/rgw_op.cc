...
void RGWListBucket::execute()
{
  ...
  if (need_container_stats()) {
    ...
    op_ret = backend->UpdateContainerStats(m);
    if (op_ret > 0) {
      bucket = m.begin()->second;
    }
  }

  RGWBackend::Bucket target(store, s->bucket_info);
  if (shard_id >= 0) {
    target.set_shard_id(shard_id);
  }

  RGWBackend::ListBucketInfo list_info;
  ...
  op_ret = backend->ListBucket(&target, list_info, max, &objs,
